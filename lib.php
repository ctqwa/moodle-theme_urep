...
function theme_urep_get_username() {
    global $COURSE, $DB, $OUTPUT;
    $context = context_course::instance($COURSE->id) ;

    $query = 'select u.id as id, firstname, lastname, firstnamephonetic, lastnamephonetic, middlename, alternatename, picture, imagealt, email from mdl_role_assignments as a, mdl_user as u where contextid=' . $context->id . ' and roleid=3 and a.userid=u.id;';

    $rs = $DB->get_recordset_sql($query);
    foreach($rs as $r) {
        echo $OUTPUT->user_picture($r, array('size' => 32, 'courseid'=>$COURSE->id));
        echo '<span>Преподаватель: </span></br>';
        echo '<a href="/user/view.php?id=' . $r->id . '&course=' . $COURSE->id . '">' . $r->firstname . ' ' . $r->lastname . '</a>';
              // echo '<a href="/user/view.php" >' $r->firstname . ' ' . $r->lastname . '</a>';
...
//     $rs = $DB->get_recordset_sql($query);
//     foreach($rs as $r) {
//        echo $OUTPUT->user_picture($r, array('size' => 24, 'courseid'=>$COURSE->id));
...
function theme_urep_set_fontwww($css) {
    global $CFG, $PAGE;
 ...
 */
function theme_urep_set_logo($css, $logo) {
    global $OUTPUT;
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_urep_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
 ...
 * @return string
 */
function urep_set_pagewidth($css, $pagewidth) {
 ...
 * @return string
 */
function urep_set_editicons($css, $editicons) {
	global $CFG;
	if (!empty($CFG->themedir)) {
		$editiconsurl = $CFG->themedir . '/urep/style/editicons.css'; //Pull the full path for autohide css
	} else {
		$editiconsurl = $CFG->dirroot . '/theme/urep/style/editicons.css'; //MDL-36065
	}
 ...
 * @return string
 */
function urep_performance_output($param) {
 ...
 */
function urep_set_customcss($css, $customcss) {
...
function theme_urep_process_css($css, $theme) {
    ...
    $css = urep_set_pagewidth($css,$pagewidth);
    ...
    $css = theme_urep_set_headingfont($css, $headingfont);
    $css = theme_urep_set_bodyfont($css, $bodyfont);
    $css = theme_urep_set_bodysize($css, $bodysize);
    $css = theme_urep_set_bodyweight($css, $bodyweight);
    ...
    $css = theme_urep_set_themecolor($css, $themecolor);
    ...
    $css = theme_urep_set_themehovercolor($css, $themehovercolor);
    ...
    $css = theme_urep_set_footercolor($css, $footercolor);
    ...
    $css = theme_urep_set_footersepcolor($css, $footersepcolor);
    ...
    $css = theme_urep_set_footertextcolor($css, $footertextcolor);
    ...
    $css = theme_urep_set_footerurlcolor($css, $footerurlcolor);
    ...
    $css = theme_urep_set_footerhovercolor($css, $footerhovercolor);
...
// Set the footer heading color.
    ...
    $css = theme_urep_set_footerheadingcolor($css, $footerheadingcolor);
    ...
    $css = theme_urep_set_slideheadercolor($css, $slideheadercolor);
    ...
    $css = theme_urep_set_slidecolor($css, $slidecolor);
    ...
    $css = theme_urep_set_slidebuttoncolor($css, $slidebuttoncolor);
    ...
    foreach (range(1, 3) as $alternativethemenumber) {
        ...
        $css = theme_urep_set_alternativecolor($css, 'color' . $alternativethemenumber, $theme->settings->{'alternativethemecolor' . $alternativethemenumber}, $default);
        $css = theme_urep_set_alternativecolor($css, 'hovercolor' . $alternativethemenumber, $theme->settings->{'alternativethemehovercolor' . $alternativethemenumber}, $defaulthover);
    }
    ...
    $css = theme_urep_set_navbarsep($css, $navbarsep);
    ...
    $css = urep_set_editicons($css, $editicons);
    ...
    $css = urep_set_customcss($css, $customcss);

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_urep_set_logo($css, $logo);
    
    // Set the background image for the page.
    $setting = 'pagebackground';
    $pagebackground = $theme->setting_file_url($setting, $setting);
    $css = theme_urep_set_pagebackground($css, $pagebackground, $setting);
    ...
    $css = theme_urep_set_defaultcategoryicon($css, $defaultcategoryicon);
    ...
    foreach (range(1, 20) as $categorynumber) {
        ...
        $css = theme_urep_set_categoryicon($css, $categoryicon, $categorynumber);
    }
    ...
    $css = theme_urep_set_slideimage($css, $slideimage, $setting);
    ...
    $css = theme_urep_set_marketingheight($css, $marketingheight);
    ...
    $css = theme_urep_set_marketingimage($css, $marketingimage, $setting);
    ...
    $css = theme_urep_set_fontwww($css);
    return $css;
}
 ...
 */
function theme_urep_initialise_colourswitcher(moodle_page $page) {
    user_preference_allow_ajax_update('theme_urep_colours', PARAM_ALPHANUM);
    $page->requires->yui_module(
        'moodle-theme_urep-coloursswitcher',
        'M.theme_urep.initColoursSwitcher',
 ...
 */
function theme_urep_get_colours($default = 'default') {
    $theme = theme_config::load('urep');
    $preference = get_user_preferences('theme_urep_colours', $default);
    foreach (range(1, 3) as $alternativethemenumber) {
 ...
 */
function theme_urep_check_colours_switch() {
    $colours= optional_param('urepcolours', null, PARAM_ALPHANUM);
    if (in_array($colours, array('default', 'alternative1', 'alternative2', 'alternative3'))) {
        set_user_preference('theme_urep_colours', $colours);
    }
}

 
function theme_urep_set_headingfont($css, $headingfont) {
...
function theme_urep_set_bodyfont($css, $bodyfont) {
...
function theme_urep_set_bodysize($css, $bodysize) {
...
function theme_urep_set_bodyweight($css, $bodyweight) {
...
function theme_urep_set_themecolor($css, $themecolor) {
...
function theme_urep_set_themehovercolor($css, $themehovercolor) {
...
function theme_urep_set_alternativecolor($css, $type, $customcolor, $defaultcolor) {
...
function theme_urep_set_footercolor($css, $footercolor) {
...
function theme_urep_set_footertextcolor($css, $footertextcolor) {
...
function theme_urep_set_footerurlcolor($css, $footerurlcolor) {
...
function theme_urep_set_footerhovercolor($css, $footerhovercolor) {
...
function theme_urep_set_footerheadingcolor($css, $footerheadingcolor) {
...
function theme_urep_set_slideheadercolor($css, $slideheadercolor) {
...
function theme_urep_set_slidecolor($css, $slidecolor) {
...
function theme_urep_set_slidebuttoncolor($css, $slidebuttoncolor) {
...
function theme_urep_set_footersepcolor($css, $footersepcolor) {
...
function theme_urep_set_navbarsep($css, $navbarsep) {
...
function theme_urep_set_pagebackground($css, $pagebackground, $setting) {
    global $OUTPUT;
...
function theme_urep_set_slideimage($css, $slideimage, $setting) {
    global $OUTPUT;
    $tag = '[[setting:'.$setting.']]';
...
function theme_urep_set_marketingheight($css, $marketingheight) {
...
function theme_urep_set_marketingimage($css, $marketingimage, $setting) {
    $tag = '[[setting:'.$setting.']]';
...
function theme_urep_set_defaultcategoryicon($css, $defaultcategoryicon) {
...
function theme_urep_set_categoryicon($css, $categoryicon, $categorynumber) {
...
function theme_urep_page_init(moodle_page $page) {
    ...
    $page->requires->jquery_plugin('tab', 'theme_urep');
    $page->requires->jquery_plugin('tooltip', 'theme_urep');
    $page->requires->jquery_plugin('transition', 'theme_urep');
    $page->requires->jquery_plugin('modernizr', 'theme_urep');  
}
