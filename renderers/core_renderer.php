 ...
 * @author     Based on code originally written by Julian (@moodleman) Ridden G J Barnard, Mary Evans, Bas Brands, Stuart Lamour and David Scotson.
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
 
 class theme_urep_core_renderer extends theme_bootstrapbase_core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = urep_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        */
        ...
        if (isloggedin() && !isguestuser()) {
            $branchlabel = $OUTPUT->user_picture($USER, array('size' => 22, 'link' => false, 'class' => 'userpicmenu')).' '.$USER->firstname.' '.$USER->lastname;
            $branchurl   = new moodle_url('/my/index.php');
            $branchtitle = get_string('mydashboard', 'theme_urep');
            $branchsort  = 10000;
            ...
            $branch->add('<em><i class="fa fa-sign-out"></i>'.get_string('logout').'</em>',new moodle_url('/login/logout.php?sesskey='.sesskey()),get_string('logout'));    
        }
    	/*
    	...
    	*/
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            if ($mycoursetitle == 'module') {
				$branchtitle = get_string('mymodules', 'theme_urep');
			} else if ($mycoursetitle == 'unit') {
				$branchtitle = get_string('myunits', 'theme_urep');
			} else if ($mycoursetitle == 'class') {
				$branchtitle = get_string('myclasses', 'theme_urep');
			} else {
				$branchtitle = get_string('mycourses', 'theme_urep');
			}
            ...
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
 			if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_urep');
        ...
        if (isloggedin() && !isguestuser()) {
            ...
            if (!empty($alternativethemes)) {
                $branchtitle = get_string('themecolors', 'theme_urep');
                $branchlabel = '<i class="fa fa-th-large"></i>' . $branchtitle;
                $branchurl   = new moodle_url('/my/index.php');
                $branchsort  = 11000;
                $branch = $menu->add($branchlabel, $branchurl, $branchtitle, $branchsort);
                
                $defaultthemecolorslabel = get_string('defaultcolors', 'theme_urep');
                $branch->add('<i class="fa fa-square colours-default"></i>' . $defaultthemecolorslabel,
                        new moodle_url($this->page->url, array('urepcolours' => 'default')), $defaultthemecolorslabel);
                foreach ($alternativethemes as $alternativethemenumber) {
                    if (!empty($this->page->theme->settings->{'alternativethemename' . $alternativethemenumber})) {
                        ...
                        $alternativethemeslabel = get_string('alternativecolors', 'theme_urep', $alternativethemenumber);
                    }
                    $branch->add('<i class="fa fa-square colours-alternative' .  $alternativethemenumber . '"></i>' . $alternativethemeslabel,
                            new moodle_url($this->page->url, array('urepcolours' => 'alternative' . $alternativethemenumber)), $alternativethemeslabel);
                }
            }
        }
    ...
    */
    ...
    public function urepblocks($region, $classes = array(), $tag = 'aside') {
...
class theme_urep_format_topics_renderer extends format_topics_renderer {
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        while ($back > 0 and empty($links['previous'])) {
            if ($canviewhidden || $sections[$back]->uservisible) {
                ...
                $previouslink .= get_string('previoussection', 'theme_urep');
        ...
        while ($forward <= $course->numsections and empty($links['next'])) {
            if ($canviewhidden || $sections[$forward]->uservisible) {
                ...
                $nextlink .= get_string('nextsection', 'theme_urep');
